...
cutoff_date = "20-07-2021  15:30:00"

set_global_seeds(seed)
np.random.seed(seed)


def dateparse1(x): return pd.datetime.strptime(x, '%Y-%m-%d %H:%M:%S')


def dateparse2(x): return pd.datetime.strptime(x, '%Y-%m-%d %H:%M:%S')


def dateparse3(x): return pd.datetime.strptime(x, '%b %d, %Y %H:%M:%S')
...
#     else:
    file = 'ETH1Y.csv'
    print('Loading file')
    df = pd.read_csv('ETH1Y.csv', parse_dates=['date'], date_parser=dateparse)
    print(df.head(1))
    print(df.tail(1))
#         df = pre_process(df, addTA)

    df = add_techicalAnalysis(df,inplace=True)
    return df
...
def train(algo, df, model_name, uniqueId, lr=None, gamma=None, noBacktest=1, cutoff_date=None, commission=0.2, addTA='Y'):
...
#         config = json.load(f)
    ...
    df = pd.read_csv('ETH1Y.csv')
    #get_data(config, portfolio=portfolio, refreshData=refreshData, addTA=addTA)

    print("\n\n\n\n\n\n\n\n\n")

    # really bad way to choose TA.
#     if addTA == 'Y':
#         df = df[['date', 'ticker', 'close', 'MOM', 'RSI', 'APO', 'HT_DCPERIOD', 'HT_DCPHASE', 'SINE','INPHASE', 'QUADRATURE', 'PPO', 'MACD', 'MACD_SIG', 'MACD_HIST', 'CMO', 'ROCP', 'TRIX', 'EMA', 'TEMA']]
...
#     else:
    ...
    uniqueId = model_name + "_"  + datetime.now().strftime("%Y%m%d %H%M%S")
...
def testSplit(df):
    ...
    cutoff_date = np.datetime64('20-07-2021  15:30:00')
