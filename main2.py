...
# def dateparse3(x): return pd.datetime.strptime(x, '%Y-%m-%d %H:%M:%S')
...
def train(algo, df, model_name, uniqueId, lr=None, gamma=None, noBacktest=1, cutoff_date=None, commission=0, addTA='N'):
    ...
    ar = datetime.datetime.strptime("2021-01-07 19:51:00", "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M")
    
    cutoff_date = np.datetime64(ar) #"2021-07-20  15:30:00" #np.datetime64(cutoff_date)
...
#         model = PPO2.load("Model1")
        ...
        model.learn(total_timesteps=round(steps), callback=[callback, checkpoint_callback])
